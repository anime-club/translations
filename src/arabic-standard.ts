export const text: text = {
  _colon: `: `,
  _comma: `، `,
  _dot: `. `,
  _exclamation: `! `,
  _hyphen: `- `,
  _question: `؟ `,
  _space: ` `,
  about: `حول المستخدم`,
  accept: `قبول`,
  acceptOrder: `قبول الطلب`,
  acceptTheOrder: `قبول الطلب`,
  account: `الحساب`,
  accountActivation: `تفعيل الحساب`,
  accountAlreadyExists: `يوجد حساب مسجّل بالفعل`,
  accountDeleted: `تم حذف الحساب`,
  accountDoesNotExist: `الحساب غير مسجّل`,
  accountManagment: `إدارة الحساب`,
  accountRecovery: `استعادة الحساب`,
  accountRemoval: `حذف الحساب`,
  add: `إضافة`,
  addAComment: `إضافة تعليق`,
  addArt: `اضف عمل فني`,
  addBlog: `اضف منشور كتابي`,
  additionalResources: `موارد إضافية`,
  addMore: `إضافة المزيد`,
  addMoreProductsToYourShop: `أضف المزيد من المنتجات إلى متجرك`,
  addNewArt: `أضف عمل فني جديد`,
  addOns: `خدمات إضافية`,
  addOnsAreOptionalEnhancementsToYourProduct: `الخدمات الإضافية هي تحسينات اختيارية لمنتجك`,
  addOnsHelpBoostIncome: `تساعد الخدمات الإضافية المشترين على تخصيص طلباتهم وقد تزيد دخلك`,
  addSignInVia: `إضافة تسجيل الدخول عبر`,
  addToCart: `إضافة إلى السلة`,
  adoptable: `شخصيات قابلة للتبنّي`,
  advancedOptions: `خيارات متقدمة`,
  agreedRevisionsWereNotCompleted: `لم يتم إكمال التعديلات المتفق عليها`,
  aiGeneratedImagesAndNeuralNetworkFiltersAreProhibited: `الصور المولدة بالذكاء الاصطناعي وفلاتر الشبكات العصبية محظورة`,
  alertsAndWidgets: `التنبيهات والأدوات`,
  all: `الكل`,
  allDevices: `كل الأجهزة`,
  allFilesYouUploadHereWillBeAvailableToTheBuyer: `جميع الملفات التي ترفعها هنا ستكون متاحة للمشتري`,
  allListings: `جميع العروض`,
  allWIPRelatedFilesShouldBeUploadedInMessages: `يجب رفع جميع ملفات العمل قيد التنفيذ في الرسائل`,
  alreadySignedIn: `تم تسجيل الدخول بالفعل`,
  amount: `المبلغ`,
  amountShouldNotBeLessThan: (...v) => `المبلغ لا يجب أن يكون أقل من ${v[0]}`,
  anAccountAlreadyUsesThisEmailSignInThenConnect: `يستخدم حساب آخر هذا البريد الإلكتروني بالفعل. سجّل الدخول بالطريقة التي تعتادها، ثم اربط هذا المزوّد من إعدادات حسابك`,
  anAccountAlreadyUsesThisEmailSignInWithPasskeyThenConnect: `يستخدم حساب آخر هذا البريد الإلكتروني بالفعل. سجّل الدخول بمفتاح المرور الخاص بك، وسنربط هذا المزوّد لك`,
  anAccountAlreadyUsesThisEmailSignInWithThenConnect: (...v) =>
    `يستخدم حساب آخر هذا البريد الإلكتروني بالفعل. سجّل الدخول باستخدام ${v[0]}، وسنربط هذا المزوّد لك`,
  animationAndVideo: `الرسوم المتحركة والفيديو`,
  animeArtByArtistsFromAllOverTheWorld: `أعمال فنية من جميع أنحاء العالم. أعمال فنية مختارة من عالم الأنمي. اختيارنا الفني عبارة عن قائمة لا نهاية لها من أفضل الأعمال الفنية للأنمي.`,
  animeArtSubscribeToArtistsLikeAndCreate: `أعمال أنمي فنية - تابع مختلف الفنانين, أبدي إعجابك وأنشئ مجموعاتك`,
  animeClubUsesToPayYouQuicklyAndKeepYourInfoSecure: (...v) =>
    `يستخدم Anime.club ${v[0]} لصرف مستحقاتك بسرعة والحفاظ على أمان معلوماتك الشخصية ومعلومات الدفع`,
  animeClubViewPostAndEarnFromAnimeContent: `أنمي.كلوب - شاهد وانشر واربح من محتوى عالم الأنمي`,
  animeCommunity: `مجتمع الأنمي`,
  animeDotclubNeverAsksForYourRecoveryCodes: `أنمي.كلوب لا يطلب أبدًا رموز الاستعادة الخاصة بك`,
  animeShopBuyAndSellProducts: `متجر الأنمي - بيع وشراء المنتجات`,
  animeSocialNetwork: `شبكة الأنمي الاجتماعية`,
  announcements: `الإعلانات`,
  appearance: `المظهر`,
  apply: `تطبيق`,
  appSettings: `إعدادات التطبيق`,
  appsVersionIs: `نسخة التطبيق`,
  areYouSureYouWantTo: `هل أنت متأكد من`,
  art: `أعمال فنية`,
  artAssociatedWithAnime: `أعمال فنية متعلقة بالأنمي`,
  artBy: (...v) => `فن من ${v[0]}`,
  artCannotBeChanged: `العمل الفني لا يمكن تغييره`,
  artGallery: `معرض فني`,
  artworkPublished: `تم نشر العمل الفني`,
  artworksAvailableToView: (...v) => `${v[0]} لوحات فنية متاحة للعرض`,
  attachedFiles: `الملفات المرفقة`,
  attentionYourOrderHasBeenDisputed: `انتباه! تم الاعتراض على طلبك. يرجى التحقق من التفاصيل`,
  authenticationFailed: `فشلت المصادقة`,
  autoPublishingToInstagramIsOnlyAvailableToInstagramBusinessAccounts: `النشر التلقائي على إنستغرام متاح فقط لحسابات الأعمال على إنستغرام`,
  avatarsAndModels: `الأفاتارات والنماذج`,
  awaitingClientReview: `بانتظار مراجعة العميل`,
  awaitingSeller: `بانتظار البائع`,
  awaitingSellersResponse: `بانتظار رد البائع`,
  back: `الرجوع`,
  backgroundAndDesign: `الخلفيات والتصميم`,
  badges: `الشارات`,
  basePrice: `السعر الأساسي`,
  beforeAcceptingPleaseReviewTheBuyersRequestCarefully: `قبل القبول، يرجى مراجعة طلب المشتري بعناية`,
  beSpecificAboutTheContent: `اذكر تفاصيل المحتوى بدقة (مثل عدد الملفات وصيغها ودقة اللوحة)`,
  beTheFirstToShareYourThoughts: `كن أول من يشارك أفكاره`,
  bigAppreciationToTheSellerForTheHardWork: `تقدير كبير للبائع على العمل الجاد`,
  block: `حظر`,
  blockUser: (...v) => `حظر @${v[0]}`,
  blog: `المدونة`,
  bookmarks: `العلامات المرجعية`,
  browseArt: `تصفح الأعمال`,
  browser: `المتصفح`,
  browseShop: `تصفح المتجر`,
  buyer: `المشتري`,
  buyerHasNotLeftAReviewYet: `لم يترك المشتري مراجعة بعد`,
  buyerReviewedTheOrder: `قام المشتري بمراجعة الطلب`,
  buyersRequest: `طلب المشتري`,
  buyersWillBeAbleToSeeThisListingYouCanHideItAgainAnytime: `سيتمكن المشترون من رؤية هذا العرض. يمكنك إخفاؤه مجددًا في أي وقت`,
  buyNow: `اشترِ الآن`,
  byContinuingYouAgreeToOur: `بالمتابعة، فإنك توافق على`,
  cancel: `إلغاء`,
  canceled: `تم الإلغاء`,
  cancelOrder: `إلغاء الطلب`,
  cannotStartWithChars: `لا يمكن البدء بـs{:"^$*[]{}()?-"!@#%&/,><':;|~\`"`,
  cantFindTheRightCategoryTellUsWhatYouWouldLikeUsToAdd: `ألا تجد الفئة المناسبة؟ أخبرنا بما تود أن نضيفه.`,
  cantSignIn: `لا يمكنك تسجيل الدخول؟`,
  categoryAndSubcategory: `الفئة والفئة الفرعية`,
  categoryAndTerms: `الفئة والشروط`,
  change: `تغيير`,
  changeArt: `تغيير العمل الفني`,
  changeAvatar: `تغيير الصورة الشخصية`,
  changedMyMind: `غيرت رأيي`,
  changeEmail: `تغيير البريد الإلكتروني`,
  changePassword: `تغيير كلمة السر`,
  changesSaved: `تم حفظ التغييرات!`,
  characterAndChibi: `الشخصيات والتشيبي`,
  chat: `دردشة`,
  checkOrderStatus: `تحقق من حالة الطلب`,
  checkOutMyShop: `تفضل بزيارة متجري`,
  chooseExistingChatOrStartANewOne: `اختر محادثةً موجودة أو ابدأ دردشة جديدة`,
  chooseTheCategoryAndSubcategoryThatBestDescribeYourProductThenSetClearTermsForBuyers: `اختر الفئة والفئة الفرعية اللتين تصفان منتجك بأفضل شكل، ثم حدّد شروطًا واضحة للمشترين.`,
  city: `المدينة`,
  clearAll: `امسح الكل`,
  clearFilters: `مسح عوامل التصفية`,
  clickTheButtonBelowToOpenYourStripeDashboard: `انقر على الزر أدناه لفتح لوحة تحكم Stripe والاطلاع على تفاصيل حسابك`,
  clickToAddANote: `انقر لإضافة ملاحظة`,
  close: `أغلق`,
  color: `اللون`,
  colorTheme: `موضوع اللون`,
  comingSoon: `قريبًا...`,
  comments: `التعليقات`,
  commercialUseAllowed: `الاستخدام التجاري مسموح`,
  commercialUseAllowsBuyersToUseYourProduct: `الاستخدام التجاري يسمح للمشترين باستخدام منتجك لأغراض تجارية مثل البيع أو الترويج لخدماتهم`,
  commercialUsePrice: `سعر الاستخدام التجاري`,
  commission: `طلب رسم`,
  commissionPublished: `تم نشر خدمة العمولة`,
  commissions: `طلبات الرسم`,
  community: `المجتمع`,
  completed: `تم الانتهاء`,
  confirmAccount: `تأكيد الحساب`,
  confirmOrder: `تأكيد الطلب`,
  confirmPassword: `تأكيد كلمة السر`,
  congratulations: `تهانينا!`,
  congratulationsOnSuccessfullyCompletingYourOrder: `تهانينا على إكمال طلبك بنجاح!`,
  connectWithUsOn: `الاتصال بنا على`,
  contactInformation: `معلومات الاتصال`,
  contactSeller: `الاتصال بالبائع`,
  contactSupport: `الاتصال بالدعم الفني`,
  contactUs: `اتصل بنا`,
  continue: `استمرار`,
  continueWithFingerprintOrFace: `تسجيل الدخول ببصمة الإصبع أو الوجه`,
  contractTitle: `عنوان العقد`,
  copiedToClipboard: `تم النسخ إلى الحافظة`,
  copy: `نسخ`,
  copyrightViolation: `انتهاك حقوق النشر`,
  couldNotCreateANewRecord: `تعذر إنشاء سجل جديد`,
  couldYouPleaseProvideTheReasonForTheCancellation: `هل يمكنك تقديم سبب الإلغاء من فضلك`,
  country: `الدولة`,
  cover: `الغلاف`,
  createAddOnsForYourProduct: `أنشئ خدمات إضافية لمنتجك`,
  createANewAccountFailed: `فشل إنشاء حساب جديد`,
  createAShopWithStripe: `أنشئ متجرك عبر Stripe`,
  createdBy: `أنشئ بواسطة`,
  createNewAccount: `إنشاء حساب جديد`,
  createPersonalizedWorkTailoredToEachRequest: `أنشئ عملًا مخصصًا يلائم كل طلب`,
  createProfile: `إنشاء ملف تعريف`,
  current: `الحالي`,
  currentPassword: `كلمة المرور الحالية`,
  daily: `يوميًا`,
  dark: `داكن`,
  decline: `رفض`,
  delete: `حذف`,
  deleteAccount: `حذف الحساب`,
  deleteArt: `حذف الرسم`,
  deletePasskey: `حذف مفتاح المرور`,
  deletePost: `حذف المنشور`,
  deleteProduct: `حذف المنتج`,
  deleteShop: `حذف المتجر`,
  deliveryTime: `وقت التسليم`,
  describeTheDetailsOfYourAddOn: `صف تفاصيل خدمتك الإضافية`,
  describeWhatsIncludedAndWhatTheBuyerCanExpectAfterPurchase: `صف ما يتضمنه المنتج وما يمكن للمشتري توقعه بعد الشراء`,
  description: `الوصف`,
  details: `التفاصيل`,
  didntReceiveEmail: `لم تتلقى البريد؟`,
  digitalDownloads: `التنزيلات الرقمية`,
  digitalDownloadsExamples: `فُرش وموارد ودروس تعليمية والمزيد`,
  discard: `تجاهل`,
  discoverAnimeArtAndArtistsFromAroundTheWorld: `اكتشف فن الأنمي وفنانيه من جميع أنحاء العالم على Anime.club. تابع المبدعين، وادعم أعمالهم، واستكشف أعمالًا فنية أصلية، أو شارك أعمالك الخاصة`,
  displayName: `اسم العرض`,
  disputed: `متنازع عليه`,
  documentation: `التوثيق`,
  donations: `التبرعات`,
  done: `انتهاء`,
  doNotShareYourRecoveryCodesWithAnyone: `لا تشارك رموز الاستعادة الخاصة بك مع أي شخص`,
  download: `تحميل`,
  downloadThemBelow: `قم بتحميلها أدناه`,
  dragAndDropOrSelectFiles: `اسحب وأفلت أو حدد الملفات`,
  drawingContest: `مسابقة الرسم`,
  dueDate: `تاريخ الاستحقاق`,
  eachProfileHasItsOwnArtShopAndFollowers: `لكل ملف تعريف أعماله الفنية ومتجره ومتابعوه. تسجيل دخول واحد يغطيها جميعًا`,
  earlier: `أقرب`,
  earningFromArts: `الربح من خلال نشر أعمالك الفنية`,
  edit: `تعديل`,
  editPost: `تعديل المنشور`,
  editProfile: `تعديل الملف الشخصي`,
  editTranslations: `تعديل الترجمات`,
  email: `البريد الإلكتروني`,
  emailAlreadyUsedByAnotherAccount: `البريد الإلكتروني قيد الاستخدام من قبل حساب آخر`,
  emailNotifications: `إشعارات البريد الإلكتروني`,
  emotesAndStickers: `الإيموتات والملصقات`,
  emotesAndStreamAssets: `الإيموتات ومواد البث`,
  enjoyYourPurchase: `استمتع بمشتراك! 😊`,
  ensureYouveUploadedTheFinalVersion: `تأكد من رفع النسخة النهائية قبل الإرسال`,
  enterOrPasteALink: `أدخل أو ألصق رابطًا`,
  enterTheAddressAssociatedWithYourAccount: `أدخل العنوان المرتبط بحسابك، وسنرسل إليك بالبريد الإلكتروني رمزًا لتسجيل الدخول وإعداد مفتاح مرور جديد`,
  enterYourPassword: `أدخل كلمة المرور الخاصة بك`,
  error: `خطأ`,
  errorPleaseContactCustomerSupport: `خطأ! الرجاء التواصل مع دعم العملاء...`,
  eventGraphics: `تصاميم الفعاليات`,
  exitWithoutSaving: `الخروج بدون حفظ`,
  extensions: `امتدادات الملف`,
  feedbackReceived: `تم استلام الملاحظات!`,
  feelFreeToShareReferenceImagesAndMaterials: `لا تتردد في مشاركة الصور المرجعية والمواد لمساعدة الفنان في إنشاء عملك`,
  files: `الملفات`,
  filesAreCorruptedOrUnusable: `الملفات تالفة أو غير قابلة للاستخدام`,
  filesForPreview: `ملفات المعاينة`,
  filesForTheBuyer: `ملفات المشتري`,
  fileSizeIsTooBig: `حجم الملف أكبر من المطلوب`,
  fileSizeIsTooSmall: `حجم الملف صغير جدًا`,
  filter: `تصفية`,
  filterBy: `تصفية على حسب`,
  findUser: `البحث عن مستخدم`,
  finishSettingUpYourShop: `إنهاء إعداد متجرك`,
  follow: `متابعة`,
  followers: `متابعون`,
  following: `تُتابع`,
  followMeOn: `تابعني على`,
  follows: `متابعون`,
  forAllOtherInquiriesPleaseContactUsAt: `لباقي الاستفسارات, اتصل بنا على:`,
  foundAnotherArtist: `وجدت فنانًا آخر`,
  fromAnimeDotClub: `من أنمي.كلوب`,
  getDetails: `الحصول على تفاصيل`,
  getStarted: `البدء`,
  graphicsAndBranding: `التصميم الجرافيكي والهوية البصرية`,
  greatNewsBuyerPurchasedYourItemTimeToCelebrate: (...v) =>
    `خبر رائع! اشترى ${v[0]} منتجك. حان وقت الاحتفال!`,
  greatNewsYourOrderIsNowInProgress: `أخبار رائعة! طلبك الآن قيد التنفيذ`,
  grid: `شبكة`,
  guestView: `وضع الضيف`,
  harassment: `التحرش`,
  hasNotAddedADescriptionFor: (...v) => `@${v[0]} لم يضف وصفًا لـ "${v[1]}"`,
  hasNotSharedAnyPublicDetailsYet: (...v) => `@${v[0]} لم يشارك أي تفاصيل عامة حتى الآن`,
  hate: `الكراهية`,
  haveAnyQuestions: `هل لديك أي أسئلة؟`,
  hereAreAFewWaysToGetThingsRolling: `إليك بعض الطرق للبدء`,
  hide: `إخفاء`,
  hideListing: `إخفاء العرض`,
  hire: `توظيف`,
  homepage: `الصفحة الرئيسية`,
  howToContactYou: `كيفية الاتصال بك`,
  howToFindYouOnOtherSocialNetworks: `كيفية العثور عليك على شبكات التواصل الاجتماعي الأخرى`,
  iAgreeWith: `أتفق على`,
  id: `المُعرف`,
  ifNoReplyIsReceivedByTimeLimitYouWillReceiveFullRefund: (...v) =>
    `إذا لم يصل رد بحلول ${v[0]}، فستسترد المبلغ كاملًا تلقائيًا`,
  ifTheresNoResponseByTimeLimit: (...v) =>
    `إذا لم يكن هناك رد بحلول ${v[0]}, سيتم وضع علامة على الطلب كمكتمل تلقائيًا`,
  ifTheresNoResponseByTimeLimitOrderWillBeCanceled: (...v) =>
    `إذا لم يصل رد بحلول ${v[0]}، فسيُلغى الطلب تلقائيًا`,
  ifYouHaveQuestionsOrConcernsPleaseContactTheSellerDirectly: `إذا كانت لديك أسئلة أو استفسارات، فيرجى التواصل مع البائع مباشرةً`,
  ifYouLoseItYoullNeedASecondWayToGetBackIn: `لديك طريقة واحدة لتسجيل الدخول. إذا فقدتها فستحتاج إلى طريقة ثانية للعودة إلى حسابك: أضف مفتاح مرور آخر، أو احفظ مجموعة من رموز الاستعادة`,
  ifYouMissTheDeadlineBuyerCanCancelForFullRefund: `إذا فاتك الموعد النهائي، يمكن للمشتري إلغاء الطلب واسترداد المبلغ كاملًا، وستُخصم أي رسوم مرتبطة بخدمة Stripe من مستحقاتك.`,
  ifYourArtIsNsfwCheckThisToAvoidAWarningOrBan: `إذا كان عملك يتضمن محتوى للبالغين، فتأكد من تحديد هذا الخيار. وإلا فقد تتلقى تحذيرًا قد يؤدي إلى حظرك.`,
  illustration: `الرسم التوضيحي`,
  imageAnimation: `تحريك الصور`,
  imgHeightNeedsToBeBiggerThanOrEqualTo: `ارتفاع الصورة يحتاج أن يكون أكبر من أو يساوي`,
  imgWidthNeedsToBeBiggerThanOrEqualTo: `عرض الصورة يحتاج أن يكون أكبر من أو يساوي`,
  impersonation: `الانتحال`,
  includeKeywordsThatBuyersWouldLikelyUseToSearchForAProductLikeYours: `تضمين الكلمات الرئيسية التي قد يستخدمها المشترون للبحث عن منتج مثل منتجك`,
  includes: `يتضمن`,
  includesXRevisionS: (...v) => `يتضمن ${v[0]} مراجعة/مراجعات`,
  increaseContrast: `زيادة التباين`,
  inProgress: `قيد التقدم`,
  introduction: `مقدمة`,
  invalidCode: `رمز غير صحيح`,
  invalidCurrentPassword: `كلمة المرور الحالية غير صالحة`,
  invalidDate: `تاريخ غير صالح`,
  invalidPartAfterAt: `جزء غير صالح بعد @`,
  invalidPartBeforeAt: `جزء غير صالح قبل @`,
  itemReviews: `مراجعات المنتج`,
  jobDetails: `تفاصيل الوظيفة`,
  joinAnimeClubCommunity: `انضم إلى مجتمع أنمي.كلوب مع الآلاف من الأشخاص النشطين والفنانين الرائعين كل يوم`,
  joinChat: `الانضمام للمحادثة`,
  joinUsOnFacebook: `انضم معنا على الفيسبوك`,
  kindlyHoldOnForTheBuyersReview: `يرجى الانتظار لمراجعة المشتري للطلب.`,
  lackOfInformation: `نقص في المعلومات`,
  language: `اللغة`,
  languages: `اللغات`,
  layout: `تخطيط`,
  leaveAReview: `اترك مراجعة`,
  legal: `قانوني`,
  legalIntro: `اقرأ الوثائق القانونية التي تحكم anime.club وتوضح كيفية التعامل مع المعلومات الشخصية`,
  letBuyersKnowIfYouOfferRevisionsAndHowMany: `أخبر المشترين إن كنت تقدم <b>تعديلات</b> وعددها`,
  letEveryoneKnowHowToDonateToYou: `اسمح للجميع بمعرفة كيفية التبرع لك`,
  letOthersKnowAboutYou: `اسمح للآخرين بمعرفة معلومات عنك`,
  letsUploadYourFirstProduct: `لنقم بتحميل منتجك الأول`,
  letUsKnowWhatsHappening: `اسمح لنا بمعرفة ما يحدث وسنتحقق من ذلك`,
  light: `فاتح`,
  likes: `إعجابات`,
  limit: `الحد`,
  listAnyContentOrRequestsYouDontAccept: `اذكر أي محتوى أو طلبات <b>لا تقبلها</b> (مثل المحتوى غير المناسب للعمل أو المحتوى السياسي أو الديني أو المسيء)`,
  listingHiddenFromBuyers: `تم إخفاء العرض عن المشترين`,
  listingIsNowVisibleToBuyers: `أصبح العرض مرئيًا للمشترين`,
  livesIn: `أعيش في`,
  loading: `جار التحميل`,
  location: `الموقع`,
  logInOrSignUp: `تسجيل الدخول أو التسجيل`,
  logoAndBrandDesign: `تصميم الشعارات والهوية البصرية`,
  logOut: `تسجيل الخروج`,
  makeSureEverythingIsReadyToGoNoEditsAfterPublishing: `تأكد من أن كل شيء جاهز — لا يمكن إجراء تعديلات بعد النشر`,
  makeSureToIncludeClearInstructions: `تأكد من تضمين تعليمات واضحة حول كيفية استخدام كل مرجع`,
  makeSureYouFullyUnderstandTheRequirements: `تأكد من فهمك الكامل للمتطلبات وقدرتك على إكمال الطلب في الموعد`,
  makeSureYourProductsArePricedCompetitively: `تأكد من أن أسعار منتجاتك تنافسية`,
  masonry: `بناء متداخل`,
  maximumFileSize: (...v) => `الحد الأقصى لحجم الملف: <b>${v[0]}</b> لكل ملف`,
  maximumNChars: (...v) => `الحد الأقصى للأحرف ${v[0]}`,
  maximumNumberOfDaysToCompleteTheOrder: `الحد الأقصى لعدد الأيام لإكمال الطلب`,
  maximumQuantity: `الحد الأقصى للكمية`,
  maxSize: `الحجم الأقصى`,
  mentionYourDeliveryTimeInDays: `اذكر <b>مدة التسليم</b> (بالأيام)`,
  menu: `القائمة`,
  message: `رسالة`,
  messageDeleted: `تم حذف الرسالة`,
  messages: `الرسائل`,
  minHeight: `الارتفاع الأدنى`,
  minimumDimensions: (...v) => `الحد الأدنى للحجم: ${v[0]}×${v[1]} بكسل`,
  minimumNChars: (...v) => `الحد الأدنى ${v[0]} حرف`,
  minorAbuseOrSexualization: `إساءة أو جنسنة طفيفة`,
  minWidth: `العرض الأدنى`,
  mixingAndMastering: `المزج والماسترينغ`,
  mobileApp: `تطبيق الجوال`,
  modelArt2D: `رسومات نماذج 2D`,
  models3D: `نماذج 3D`,
  monthly: `شهريًا`,
  moreFrom: (...v) => `المزيد من خلال ${v[0]}`,
  musicAndAudio: `الموسيقى والصوت`,
  musicAndShortFormVideo: `الموسيقى والفيديوهات القصيرة`,
  mustBeNChars: (...v) => `لا بد أن تكون ${v[0]} حرف`,
  myPayments: `مدفوعاتي`,
  myProfile: `ملفي الشخصي`,
  mySettings: `إعداداتي`,
  name: `الاسم`,
  namePasskey: `تسمية مفتاح المرور`,
  nameYourDevicesToTellThemApart: `أعطِ أجهزتك أسماء لتتمكن من التمييز بينها`,
  need1Lowercase: `يلزم 1 حرف صغير`,
  need1Number: `يلزم 1 رقم`,
  need1Uppercase: `يلزم 1 حرف كبير`,
  needAtLeast1CharacterFromN: (...v) => `يلزم على الأقل 1 حرف كبير من [${v[0]}]`,
  needNCharacter: (...v) => `يلزم ${v[0]} حرف`,
  needsYourReview: `بحاجة إلى مراجعتك`,
  new: `جديد`,
  newAccount: `حساب جديد`,
  newArt: `عمل فني جديد`,
  newArtworks: `أعمال فنية جديدة`,
  newBlog: `منشور جديد`,
  newCommission: `طلب رسم جديد`,
  newOrder: `طلب جديد`,
  newPassword: `كلمة سر جديدة`,
  newProduct: `منتج جديد`,
  newsFeed: `شريط الأخبار`,
  next: `التالي`,
  nextTimeYouAuthenticateYouCouldUseThe: `في المرة القادمة التي تقوم فيها بالمصادقة، يمكنك استخدام الحساب المتصل بدلاً من تسجيل الدخول وكلمة المرور`,
  no: `لا`,
  noActiveOrdersToShow: `لا توجد طلبات نشطة للعرض`,
  noCommentsJustYet: `لا توجد تعليقات بعد`,
  noInternetConnection: `لا يوجد اتصال بالانترنت!`,
  nonConsensualIntimateContent: `محتوى حميم غير مرغوب فيه`,
  none: `لا شيء`,
  noNotifications: `لا توجد إشعارات`,
  noOrders: `لا توجد طلبات`,
  noResults: `لا توجد نتائج`,
  noReviewsJustYet: `لا توجد مراجعات بعد`,
  noSalesYetNoWorries: `لا توجد مبيعات بعد؟ لا تقلق، أنت في البداية فقط`,
  notAcceptingOrders: `لا يقبل الطلبات`,
  noteOnlyVisibleToYou: `ملاحظة (مرئية لك فقط)`,
  notes: `ملاحظات`,
  nothingIsChargedUntilTheSellerAccepts: `لا يتم خصم أي مبلغ حتى يقبل البائع، وإذا رفض فلن يتم خصم أي شيء منك`,
  notifications: `الإشعارات`,
  nsfwContent18Plus: `محتوى للبالغين (18+)`,
  NSFWIndicatesAdultContent: `18+ يشير NSFW إلى محتوى للبالغين. اختر هذا الحقل فقط إذا كان منتجك مخصصًا للبالغين`,
  numberOfAdjustmentsAClientCouldRequest: `عدد التعديلات التي يمكن للعميل طلبها`,
  numFilesIncluded: (...v) => `عدد الملفات المضمنة: <b>${v[0]}</b>`,
  numOfNewComments: (...v) => `تعليقات جديدة: ${v[0]}`,
  numOfNewReactions: (...v) => `تفاعلات جديدة: ${v[0]}`,
  numOfReplies: (...v) => `الردود: ${v[0]}`,
  numOfReviews: (...v) => `المراجعات: ${v[0]} `,
  offerDigitalProducts: `اعرض منتجات رقمية يمكن للمشترين الوصول إليها فورًا`,
  offerOptionalAddOns: `قدّم خدمات إضافية اختيارية مثل <b>التسليم العاجل</b> أو <b>شخصية إضافية</b> أو <b>خلفية مفصلة</b>`,
  online: `متصل`,
  onlyLatinLettersNumbersHyphen: `فقط الأحرف اللاتينية والأرقام والشرطة والشرطة السفلية والنقطة`,
  onlyNumbers: `أرقام فقط`,
  onlyYouCanSeeWhatYouveSaved: `فقط أنت يمكنك رؤية ما حفظته`,
  open: `مفتوح`,
  openDispute: `فتح نزاع`,
  openDisputeChat: `فتح محادثة النزاع`,
  openDisputeGuidance: `اشرح بوضوح ما الخطأ في الطلب وما الحل الذي تتوقعه. بعد فتح النزاع، سننشئ محادثة جماعية تضمك والبائع وفريق دعم anime.club. يمكن للطرفين تقديم الأدلة فيها. يستطيع فريق الدعم الوصول إلى تفاصيل الطلب وجميع الملفات التي تمت مشاركتها بالفعل ضمن الطلب.`,
  openGuestView: `افتح وضع الضيف`,
  openingSoon: `قريبًا الافتتاح!`,
  openToWork: `مفتوح للعمل`,
  optional: `اختياري`,
  optionallyYouCanAddADescriptionToShareMoreAboutYourCreation: `يمكنك اختياريًا إضافة وصف لمشاركة المزيد عن عملك`,
  or: `أو`,
  order: `طلب`,
  orderAgain: `أعد الطلب مرة أخرى`,
  orderCanceledTheBuyersPaymentWasReleased: `تم إلغاء الطلب. حررنا المبلغ المحجوز على بطاقة المشتري، وقد يستغرق بنكه بضعة أيام`,
  orderCanceledYourPaymentWasReleased: `تم إلغاء الطلب. حررنا المبلغ المحجوز على بطاقتك، وقد يستغرق بنكك بضعة أيام`,
  orderCompleted: `تم إكمال الطلب 🎉`,
  orderConfirmed: `تم تأكيد الطلب`,
  orderDetails: `تفاصيل الطلب`,
  ordered: (...v) => `تم الطلب ${v[0]}`,
  orderedByMistake: `طلبته بالخطأ`,
  orderOptions: `خيارات الطلب`,
  orderStatus: `حالة الطلب`,
  originalAnimation: `رسوم متحركة أصلية`,
  originalMusic: `موسيقى أصلية`,
  otherOrderIssue: `مشكلة أخرى في الطلب`,
  otherReason: `سبب آخر`,
  outOfStock: `غير متوفر`,
  overlaysAndPanels: `التراكبات واللوحات`,
  packagingAndCovers: `العبوات والأغلفة`,
  pageNotFound: `الصفحة غير موجودة`,
  passkey: `مفتاح مرور`,
  passkeys: `مفاتيح المرور`,
  password: `كلمة السر`,
  paymentProcessingFee: `رسوم معالجة الدفع`,
  payments: `المدفوعات`,
  payouts: `المدفوعات`,
  personal: `شخصي`,
  personalCircumstances: `ظروف شخصية`,
  platform: `منصة`,
  platformFee: `رسوم المنصة`,
  platformPoliciesMayStillApply: `قد تظل سياسات المنصة سارية`,
  pleaseAllowPopups: `يرجى السماح بالنوافذ المنبثقة`,
  pleaseCheckYour: (...v) => `يرجى التحقق من ${v[0]} الخاص بك`,
  pleaseEnterANumberBetweenXandX: (...v) => `يرجى إدخال رقم بين ${v[0]} و ${v[1]}`,
  pleaseEnterAtLeastNChars: (...v) => `الرجاء إدخال ${v[0]} أحرف على الأقل`,
  pleaseFillOutYourDetails: `الرجاء تعبئة بياناتك`,
  pleaseFixTheErrorsBeforeSubmitting: `يرجى تصحيح الأخطاء قبل الإرسال`,
  pleaseProvideATitleForYourArtwork: `يرجى إدخال عنوان لعملك الفني`,
  pleaseRateYourOrder: `يرجى تقييم طلبك`,
  pleaseRespondToTheOrderBy: (...v) => `يرجى الرد على الطلب قبل ${v[0]}`,
  pleaseReviewTheBuyersReferenceFiles: `يرجى مراجعة ملفات المراجع الخاصة بالمشتري واتباعها كما هو موضح في وصف الطلب`,
  pleaseReviewTheDetailsCarefullyBeforeProceeding: `يرجى مراجعة التفاصيل بعناية قبل المتابعة`,
  pleaseSignInToYourAccount: `الرجاء تسجيل الدخول لحسابك`,
  pleaseStartTyping: `يرجى البدء في الكتابة`,
  pleaseSubmitTheOrderBy: (...v) => `يرجى تسليم الطلب قبل ${v[0]}`,
  pleaseTakeAMomentToReviewTheOrder: `يرجى أخذ لحظة لمراجعة الطلب`,
  pleaseTryAgainOrClick: (...v) => `يرجى المحاولة مرة أخرى أو النقر فوق ${v[0]}`,
  pleaseUploadOnlyFinishedWorkHere: `يرجى رفع الأعمال المنتهية فقط هنا`,
  pleaseVerifyYourAccount: `الرجاء تأكيد حسابك`,
  portfolio: `معرض الأعمال`,
  pressHereTo: `أضغط هنا من أجل`,
  price: `السعر`,
  privacyPolicy: `سياسة الخصوصية`,
  privateInformation: `خاص`,
  proceedToCheckout: `المتابعة للدفع`,
  product: `المنتج`,
  productPublished: `تم نشر المنتج`,
  profile: `ملف التعريف`,
  profiles: `ملفات التعريف`,
  prohibitedTransaction: `معاملة محظورة`,
  promotions: `العروض الترويجية`,
  publicInformation: `عام`,
  publishOn: (...v) => `النشر على ${v[0]}`,
  purchases: `المشتريات`,
  rankings: `التصنيفات`,
  reactiveAvatars: `أفاتارات تفاعلية`,
  reason: `السبب`,
  recoveryCode: `رمز الاستعادة`,
  recoveryCodes: `رموز الاستعادة`,
  recoveryCodesLeft: (...v) => `بقي ${v[0]}`,
  referenceSheetsAndComics: `أوراق مرجعية وقصص مصورة`,
  removeFilter: (...v) => `إزالة عامل التصفية: ${v[0]}`,
  reorderFiles: `إعادة ترتيب الملفات`,
  replies: `الردود`,
  reply: `رد`,
  replyToUsername: (...v) => `الرد على ${v[0]}`,
  report: `الإبلاغ`,
  requestDetails: `تفاصيل الطلب`,
  review: `مراجعة`,
  reviewed: `تمت المراجعة`,
  reviews: `المراجعات`,
  reviewTitle: `عنوان المراجعة`,
  revisions: `المراجعات`,
  rigging2D: `تجهيز نماذج 2D للتحريك`,
  rules: `القواعد`,
  sales: `المبيعات`,
  save: `حفظ`,
  saveAsTemplate: `حفظ كقالب`,
  saveAvatar: `حفظ الصورة الشخصية`,
  saveChanges: `حفظ التغييرات`,
  schedulesAndSocialGraphics: `الجداول وتصاميم وسائل التواصل`,
  search: `بحث`,
  searchProducts: `البحث عن المنتجات`,
  selectAnime: `اختر أنمي`,
  selectAppearanceContrastAndColorTheme: `اختر المظهر والتباين ونسق الألوان`,
  selectAvatar: `اختر صورة شخصية`,
  selectCharacters: `اختر شخصية`,
  selectFile: `اختر ملف`,
  selectThumbnail: `اختر صورة مصغرة`,
  selfHarmOrSuicide: `الإيذاء الذاتي أو الانتحار`,
  sellDownloadableContent: `بع محتوى قابلًا للتنزيل`,
  seller: `البائع`,
  sellerHasntResponded: `البائع لم يرد`,
  sellerNet: `صافي مستحقات البائع`,
  sellerTermsOfService: `شروط خدمة البائع`,
  send: `إرسال`,
  sendEmail: `ارسل رسالة بريد`,
  sendSuggestion: `إرسال الاقتراح`,
  sessions: `الجلسات`,
  settings: `الإعدادات`,
  settingUpAShopIsFreeAndOnlyTakesAFewMinutes: `إعداد متجر مجاني ويستغرق بضع دقائق فقط`,
  setUpStripeAccountToGetPaidWithAnimeClub: `أنشئ حسابًا على Stripe لتلقي مستحقاتك من Anime.club`,
  share: `مشاركة`,
  sharedFilesAreNotConsideredDelivery: `الملفات المشاركة لا تُعد تسليمًا`,
  shareYourArt: `شارك فنك`,
  shareYourArtWithTheWorld: `شارك فنك مع العالم`,
  shareYourFavoritePiecesOn: (...v) => `شارك أعمالك المفضلة على ${v[0]}`,
  shareYourRulesAndTheTypesOfRequestsYouDontAccept: `شارك قواعدك وأنواع الطلبات التي لا تقبلها`,
  shareYourShopOnSocialMedia: `شارك متجرك على وسائل التواصل الاجتماعي`,
  shareYourStory: `شارك قصتك!`,
  sharingPersonalInformation: `مشاركة المعلومات الشخصية`,
  shop: `تسوق`,
  shopOrders: `طلبات المتجر`,
  shopReviews: `مراجعات المتجر`,
  show: `إظهار`,
  showLess: `عرض أقل`,
  showListing: `إظهار العرض`,
  showMore: `عرض أكثر`,
  showMyTermsOfService: `عرض شروط الخدمة الخاصة بي`,
  showSellerTermsOfService: `عرض شروط الخدمة الخاصة بالبائع`,
  signIn: `تسجيل دخول`,
  signInWithTheDeviceYouAlreadyUnlock: `سجّل الدخول من الجهاز الذي تفتحه أساسًا، واحفظ رموز الاستعادة في حال فقدته`,
  signOutEverywhere: `تسجيل الخروج من كل الأجهزة`,
  size: `الحجم`,
  skip: `تخطى`,
  socialMedia: `وسائل التواصل الاجتماعي`,
  soundEffects: `المؤثرات الصوتية`,
  spaceCharacterIsNotAllowed: `غير مسموح بالمسافة`,
  spam: `البريد المزعج`,
  storyboards: `لوحات القصة`,
  submitAReport: `إرسال تقرير`,
  submitWork: `إرسال العمل`,
  subscribeToTalentsToKeepUpWithTheirProgress: `اشترك في صفحات المواهب لمتابعة إنجازاتهم`,
  successChangedPasswordPleaseSignIn: `تم بنجاح تم تغيير كلمة السر. الرجاء تسجيل الدخول`,
  successCreatedRecord: `تم بنجاح إنشاء السجل!`,
  successfullyDownloaded: `تم التحميل بنجاح!`,
  successfullyUploaded: `تم الرفع بنجاح!`,
  suggestACategoryOrSubcategory: `اقترح فئة أو فئة فرعية`,
  supportedFormats: `الصيغ المدعومة:`,
  supportMeOn: `ادعمني على`,
  switchedTo: (...v) => `تم التبديل إلى ${v[0]}`,
  switchProfile: `تبديل ملف التعريف`,
  system: `النظام`,
  takeCustomRequests: `استقبل طلبات مخصصة`,
  tapAddToCreateOne: `اضغط على «إضافة» لإنشاء واحد`,
  tax: `الضريبة`,
  tellTheBuyersWhatTheyWillGet: `أخبر المشترين بما سيحصلون عليه بالضبط، مثل عدد الرسومات أو صيغ الملفات أو مواصفات اللوحة كالدقة وكثافة النقاط DPI`,
  tellUsAboutYourself: `اخبرنا عن نفسك`,
  termsAndConditions: `الأحكام والشروط`,
  termsOfUse: `شروط الاستخدام`,
  thankYouForReport: `شكرًا لمساعدتك في الحفاظ على مجتمع آمن ومحترم عبر الإنترنت`,
  thankYouForYourPurchase: `شكرًا لشرائك!`,
  thankYouForYourReview: `شكرًا لمراجعتك!`,
  theBuyerAcceptedThemAtCheckout: `وافق المشتري عليها عند إتمام الدفع`,
  theBuyerCanSeeTheseFilesRightAway: `يمكن للمشتري رؤية هذه الملفات على الفور`,
  theLinkIsBrokenOrPageHasBeenRemoved: `الرابط معطل أو تمت إزالة الصفحة`,
  theMaximumQuantityThatACustomerCanAdd: `الحد الأقصى للكمية التي يمكن للعميل إضافتها`,
  theReasonYouProvideForCancellingThisOrderWillBeSharedWithTheBuyer: `ستتم مشاركة سبب إلغاء هذا الطلب الذي تقدمه مع المشتري`,
  theReasonYouProvideForCancellingThisOrderWillBeSharedWithTheSeller: `سيتم مشاركة سبب الإلغاء الذي تقدمه مع البائع`,
  theReviewPeriodIsLimitedtoNdays: (...v) => `فترة المراجعة محدودة بـ ${v[0]} أيام`,
  theseAreYourTermsForThisItem: `هذه هي شروطك الخاصة بهذا العنصر`,
  theSellerCanSeeTheseFilesRightAway: `يمكن للبائع رؤية هذه الملفات على الفور`,
  theSellerConfirmedYourOrderAndIsWorkingOnIt: `<b>أكّد</b> البائع طلبك وهو يعمل عليه الآن`,
  theSellerHasUploadedTheseFilesForYou: `قام البائع برفع هذه الملفات لك`,
  theseRulesAreProvidedByTheSellerAndApplyOnlyToThisItem: `هذه القواعد مقدمة من البائع وتنطبق على هذا المنتج فقط`,
  thisCannotBeUndone: `لا يمكن التراجع عن هذا`,
  thisDevice: `هذا الجهاز`,
  thisFieldIsOptional: `هذا الحقل اختياري`,
  thisIsACustomOrderNotAReadyMadeProduct: `هذا طلب مخصص — وليس منتجًا جاهزًا. يبدأ البائع العمل عليه بعد طلبه`,
  thisIsThePriceYouPropose: `هذا هو السعر الذي تقترحه`,
  thisListingWontBeVisibleToBuyersAnymore: `لن يعود هذا العرض مرئيًا للمشترين — ولكن يمكنك إظهاره مجددًا متى شئت`,
  threateningViolence: `تهديد العنف`,
  thumbnailsAndBanners: `الصور المصغرة واللافتات`,
  timeConstraints: `قيود الوقت`,
  title: `العنوان`,
  titleYourAddOn: `سمِّ خدمتك الإضافية`,
  today: `اليوم`,
  toGetWorkRelatedNotifications: `للحصول على إشعارات تتعلق بالعمل`,
  toHelpTheCreatorCompleteYourOrder: `لمساعدة المنشئ في إكمال طلبك بما يرضيك، يرجى تقديم وصف تفصيلي لما تريده. قم بتضمين الموضوع المطلوب، النمط، وميزات الشخصية، وأي مواصفات مهمة أخرى`,
  tooManyAttempts: `محاولات كثيرة جدًا، يُرجى الانتظار قليلًا ثم المحاولة مرة أخرى`,
  total: `الإجمالي`,
  toViewThisDataYouNeedToUnblock: (...v) => `لعرض هذه البيانات، تحتاج إلى إلغاء حظر @${v[0]}`,
  trademarkViolation: `انتهاك العلامة التجارية`,
  transitionsAndProps: `الانتقالات والإكسسوارات`,
  typeYourTextHere: `ادخل النص هنا...`,
  unblock: `إلغاء الحظر`,
  unblockUser: (...v) => `إلغاء حظر @${v[0]}`,
  unfollow: `إلغاء المتابعة`,
  unfortunatelyTheSellerHadToCancelThisOrder: (...v) => `للأسف، اضطر ${v[0]} لإلغاء هذا الطلب`,
  unlimited: `غير محدود`,
  upload: `تحميل`,
  uploadedFilesLocked: `رفعت هذه الملفات عند إنشاء المنشور. وهي مقفلة الآن، لكنها لا تزال ظاهرة أدناه`,
  uploadNew: `تحميل جديد`,
  uploadNewPost: `تحميل منشور جديد`,
  uploadNewProduct: `تحميل منتج جديد`,
  uploadTheFilesYourBuyerWillGet: `ارفع الملفات التي <b>سيحصل عليها المشتري</b>`,
  uploadUpToNumberFiles: (...v) => `يمكنك رفع ما يصل إلى <b>${v[0]}</b> ملفات`,
  username: `اسم المستخدم`,
  usernameAlreadyExists: `اسم المستخدم مستخدم من قبل`,
  usernameAndXMoreCommented: (...v) => `علّق <b>${v[0]}</b> و${v[1]} آخرون`,
  usernameAndXMoreFollowYou: (...v) => `<b>${v[0]}</b> و ${v[1]} يتابعونك`,
  usernameAndXMoreReacted: (...v) => `<b>${v[0]}</b> و${v[1]} آخرين تفاعلوا`,
  usernameAndXMoreRepliedToYourComment: (...v) => `رد <b>${v[0]}</b> و${v[1]} آخرون على تعليقك`,
  usernameCommentedOnYourWork: (...v) => `علّق <b>${v[0]}</b> على عملك`,
  usernameHasCanceledTheOrder: (...v) => `<b>${v[0]}</b> قد ألغى الطلب`,
  usernameHasntAddedAnythingToTheirPortfolioYet: (...v) =>
    `لم يضف @${v[0]} أي شيء إلى معرض أعماله بعد`,
  usernameHasntListedAnyProductsForSaleYet: (...v) => `لم يعرض @${v[0]} أي منتجات للبيع بعد`,
  usernameHasSubmittedTheWork: (...v) => `<b>${v[0]}</b> قام بتسليم العمل`,
  usernameLeftAReaction: (...v) => `ترك <b>${v[0]}</b> تفاعلًا${v[1] ? ` ${v[1]}` : ``}`,
  usernameRepliedToYourComment: (...v) => `رد <b>${v[0]}</b> على تعليقك`,
  usernameStartedFollowingYou: (...v) => `بدأ <b>${v[0]}</b> بمتابعتك`,
  useThisEmailTo: `استخدم هذا البريد الإلكتروني للمصادقة وتلقي الإشعارات`,
  useYourFingerprintFaceOrScreenLock: `استخدم بصمة إصبعك أو وجهك أو قفل الشاشة`,
  valueIsNotTheSame: `القيمة ليست هي نفسها`,
  verificationCode: `رمز التأكيد`,
  verifyCode: `أكد الرمز`,
  videoEditing: `مونتاج الفيديو`,
  viewDetails: `عرض التفاصيل`,
  viewEveryNotificationOfAGroupSeparately: `عرض كل إشعار من مجموعة بشكل منفصل`,
  viewReview: `عرض المراجعة`,
  views: `المشاهدات`,
  visitOurDiscordChannelToGetMoreDetails: `قم بزيارة قناتنا في ديسكورد للحصول على مزيد من التفاصيل`,
  visitStripeDashboard: `زيارة لوحة تحكم Stripe`,
  vocalTuning: `ضبط الغناء`,
  voiceOver: `التعليق الصوتي`,
  vote: `تصويت`,
  vroidAnd3DAddOns: `إضافات VRoid و3D`,
  weAreExcitedToSeeYourProgressKeepItUp: `نحن متحمسون لرؤية تقدمك. استمر في ذلك!`,
  weAreOnInstagram: `تجدنا على انستقرام`,
  weAreWorkingOnIt: `نعمل على الأمر`,
  weDoNotDisplayNSFWContentYet: `لا نعرض محتوى للبالغين بعد`,
  weekly: `أسبوعيًا`,
  weHaveSentTooManyEmails: `لقد أرسلنا الكثير من الرسائل الإلكترونية، يرجى المحاولة مرة أخرى في وقت لاحق أو الاتصال بخدمة العملاء`,
  welcomeToAnimeClub: `أهلًا بك في أنمي.كلوب!`,
  wellDoneWorkHasBeenSuccessfullyCompleted: `عمل رائع! تم إكمال العمل بنجاح`,
  wellReviewYourSuggestionWithinOneWeekAndLetYouKnowTheOutcome: `سنراجع اقتراحك خلال أسبوع واحد ونُطلعك على النتيجة.`,
  weSentAnEmailTo: `أرسلنا بريد إلكتروني إلى:`,
  weSentYouANewVerificationEmailPleaseEnterTheCode: `قمنا بإرسال رسالة تأكيد جديدة على البريد. الرجاء إدخال رمز التأكيد`,
  weSupportOnlyTheFollowingFileExtensions: `ندعم فقط امتدادات الملفات التالية:`,
  weWouldLoveToHearFromYou: `نود أن نسمع منك`,
  whatWouldYouLikeToCreate: `ماذا تود أن تنشئ؟`,
  whenYouPurchaseSomethingItWillAppearHere: `عند شراء شيء ما، سيظهر هنا`,
  work: `عمل`,
  workDescription: `وصف العمل`,
  workDoesNotMatchRequest: `العمل لا يطابق الطلب`,
  workIsIncompleteOrFilesAreMissing: `العمل غير مكتمل أو الملفات مفقودة`,
  workIsNotOriginalOrUsesAI: `العمل غير أصلي أو يستخدم الذكاء الاصطناعي`,
  wrongEmailOrPassword: `البريد الإلكتروني أو كلمة المرور غير صحيحين`,
  ych: `YCH`,
  yes: `نعم`,
  yesterday: `أمس`,
  youAreAlreadyAuthenticated: `سبق وقمنا بالتحقق من حسابك`,
  youCanAutoPublishToYourSocialMediaAccount: `يمكنك النشر التلقائي على حسابات التواصل الاجتماعي الخاصة بك`,
  youCanceledTheOrder: `لقد ألغيت الطلب`,
  youCanChooseFromTheListOrEnterACustomReason: `يمكنك الاختيار من القائمة أو إدخال سبب مخصص`,
  youCanModifyTheTypesOfNotificationsYouReceiveViaEmail: `يمكنك تعديل أنواع الإشعارات التي تتلقاها عبر البريد الإلكتروني`,
  youCanRequestRemovalOfYourAccount: `يمكنك طلب إزالة حسابك مع جميع ملفاته الشخصية وبياناتك`,
  youCanStillNavigateToThePreviouslyOpenedPages: `لا يزال بإمكانك التنقل للصفحات المفتوحة مسبقًا`,
  youDidNotSelectAnyChats: `لم تُحدد أي محادثات`,
  youDidNotUpdateAnyValues: `لم تقم بتحديث أي قيم`,
  youDisputedTheOrder: `تم فتح النزاع. تحقق من الرسائل لمحادثة النزاع`,
  youDoNotHaveAnyChats: `ليس لديك أي محادثات`,
  youHaveNoPurchases: `ليس لديك أي مشتريات`,
  youHaveNotSetAPassword: `لم تقم بتعيين كلمة مرور`,
  youHaveNotSharedAnythingYet: `لم تشارك أي شيء حتى الآن`,
  youHaveReachedTheMaximumNumberOfFiles: `لقد وصلت إلى الحد الأقصى لعدد الملفات`,
  youHaveSuccessfullyCreatedYourShop: `لقد أنشأت متجرك بنجاح`,
  youllSeeReviewsHereOnceBuyersShareTheirExperience: `سترى المراجعات هنا بمجرد أن يشارك المشترون تجربتهم`,
  youMustPressSubmitButtonToFinalizeTheOrder: `يجب الضغط على زر التسليم لإتمام الطلب`,
  yourAccountIsVerifiedPleaseSignIn: `تم التحقق من حسابك. الرجاء تسجيل الدخول`,
  yourAccountWillBeRemoved: (...v) => `سيتم إزالة حسابك ${v[0]}`,
  yourArts: `أعمالك الفنية`,
  yourAvatar: `صورتك الشخصية`,
  yourBlog: `مدونتك`,
  youReceivedANewOrderFrom: (...v) => `لقد تلقيت طلبًا جديدًا من <b>${v[0]}</b>`,
  youReceivedAReview: `لقد استلمت مراجعة`,
  yourFansAreWaiting: `معجبيك في انتظارك`,
  yourFavoritesWillShowUpHereAddSome: `مفضلاتك ستظهر هنا. أضف بعضها!`,
  yourFeedbackGuidesBuyersAndHelpsSellersOfferBetterProducts: `ملاحظاتك ترشد المشترين وتساعد البائعين على تقديم منتجات أفضل`,
  yourFeedIsEmpty: `شريطك الإخباري فارغ`,
  yourSessionHasExpiredPleaseSignInAgain: `انتهت جلستك, الرجاء تسجيل الدخول مجددًا`,
  yourSessionWasEndedPleaseSignInAgain: `تم إنهاء جلستك، الرجاء تسجيل الدخول مجددًا`,
  yourTermsOfService: `شروط الخدمة الخاصة بك`,
  youWillBeAbleToDownloadYourFilesImmediatelyAfterPurchase: `ستتمكن من تنزيل ملفاتك فور الشراء—دون انتظار!`,
  youWillBeAbleToUploadReferenceFilesAfterCheckout: `يمكنك رفع الملفات المرجعية بعد إتمام الدفع`,
}
